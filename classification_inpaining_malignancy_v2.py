...
parser.add_argument('both', type=str, nargs='?')
...
for fold in range(folds):
    ...
    if args.both == 'both':
        ...
        sampler = class_imbalance_sampler(y_train-1)
        dataset_train = Dataset_malignacy(X_train, y_train, path_to_use, transform=True)
        dataset_test = Dataset_malignacy(X_test, y_test, path_to_use, transform=False)
        dataset_val = Dataset_malignacy(X_val, y_val, path_to_use, transform=False)
        dataloader_train = DataLoader(dataset_train, batch_size=32, sampler=sampler)
        dataloader_test = DataLoader(dataset_test, batch_size=32)
        dataloader_val = DataLoader(dataset_val, batch_size=32)
